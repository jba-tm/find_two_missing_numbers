...
def find_two_missing_numbers(arr: list):
    ...
    result = ((n * (n + 1)) / 2 - get_sum(arr, n - 2))
    ...
    for i in range(0, n - 2):
        if arr[i] <= avg:
            sum_smaller_half += arr[i]
        else:
            sum_greater_half += arr[i]
    ...
    # equal to avg)
    total_smaller_half = (avg * (avg + 1)) / 2
    first_number = total_smaller_half - sum_smaller_half

    # The first (smaller) element = (sum
    # of natural numbers from avg+1 to n) -
    # (sum of array elements greater than avg)
    second_number = ((n * (n + 1)) / 2 - total_smaller_half) - sum_greater_half
